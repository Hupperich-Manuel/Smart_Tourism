...
def SecondUser(request):
    ...
    building_index, evaluation = model.rank_items(userid = request.user, user_items = X.T.tocsr(),selected_items = Question.objects.values('building'))
    ...
    for index in building_index[6:11]:
        place = Question.objects.get(id=index).values()
        result.append(place[0])
        coordinates_start.append([place[1], place[2]])
        descriptions.append(descriptions[-2])
        descript_places.append(descript_places[-2])
        links.append(place[-1])
